 ...
 * @LastEditTime: 2022-12-07 13:23:20
 * @Copyright: [版权] 2022  Creator CO.LTD. All Rights Reserved.
 * @Descripttion: 
 */
module.exports = {
    'name': 'cc-obfuscated-3_x 混淆',
    'description': "代码混淆工具, 支持项目构建后自动混淆代码, <br>支持微信小游戏和抖音小游戏代码混淆, 支持分包的代码混淆, <br>可在项目设置面板调整混淆参数, 可在构建面板设置混淆参数, <br>目前各个面板表层的显示层暂不同步, <br> 保存的混淆 JSON 文件是同步的, 最终混淆参数都用的 JSON 的参数, <br> 目前已支持 AST 抽象语法树进行混淆(可独立混淆), <br>采用 MD5 保证每次混淆的结果都是唯一且不可逆的",
    'menu': {
        name: "代码混淆·✍",
        argu_web_set: "混淆参数·WEB-试验面板",
        argu_set: "混淆参数·设置面板",
        argu_des: "选择 JS ·指定混淆·面板",
        change_uuid: "一键更改 UUID",
        open: "✔ 开启混淆",
        close: "✘ 关闭混淆",
